...
segment_target_duration=4       # try to create a new segment every 10 seconds
...
for rendition in "${renditions[@]}"; do
  ...
  cmd+=" -hls_segment_filename ${target}/${target}_${name}_%03d.ts ${target}/${target}_${name}.m3u8"
...
if [ $resolutionValid -eq 1 ]; then
  ...
  echo -e "${master_playlist}" > ${target}/${target}_playlist.m3u8
